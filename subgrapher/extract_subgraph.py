    ...
    for n in t.traverse_postorder():
        for c in n.children:
            if c.is_leaf():
                ...
                error_prob = 1 - max((float(c.label) - NORMALIZER), 0) / (
                    1 - NORMALIZER
                )
...
def contract_node(g, u):
    ...
    g.add_edge(
        ...
        num_edges=num_edges1 + num_edges2,
    )
...
def nx2pyg(G):
    ...
    edge_index = (
        torch.tensor(
            [[node2id[e[0]], node2id[e[1]]] for e in G.edges()], dtype=torch.long
        )
        .t()
        .contiguous()
    )
